...
class StoreDebtRequest extends FormRequest
{
    ...
    {
        $sync = $this->boolean('sync_recurrence');

        return [
            'title' => ['required', 'string', 'max:200'],
            'creditor' => ['nullable', 'string', 'max:200'],
            'debt_type' => ['required', Rule::enum(DebtType::class)],
            'principal_amount' => ['required', 'numeric', 'min:0.01', 'decimal:0,2'],
            'balance_amount' => ['nullable', 'numeric', 'min:0', 'decimal:0,2'],
            'currency' => ['nullable', 'string', 'size:3'],
            'status' => ['nullable', Rule::enum(DebtStatus::class)],
            'financial_account_id' => [
                'nullable',
                Rule::requiredIf($sync),
                'uuid',
            ...
            'sync_recurrence' => ['sometimes', 'boolean'],
        ];
    }
}
